...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (! Schema::hasColumn('oauth_access_tokens', 'workspace_id')) {
        ...
        DB::table('users')->orderBy('id')->each(function (object $user) use ($now): void {
            $existingMembership = DB::table('workspace_memberships')
        ...
        $workspaceIdColumn = collect(Schema::getColumns('projects'))
            ->firstWhere('name', 'workspace_id');

        if ($workspaceIdColumn && $workspaceIdColumn['nullable']) {
            Schema::table('projects', function (Blueprint $table) {

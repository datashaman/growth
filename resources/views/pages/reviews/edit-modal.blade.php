...
new class extends Component {
    ...
    public function mount(string $reviewId): void
    {
        ...
        $this->held_at = $review->held_at?->format('Y-m-d') ?? '';
        $this->decision = $review->decision;
        $this->owner_role_id = $review->owner_role_id;
        $this->review_plan_id = $review->review_plan_id;
        $this->entry_criteria_text = is_array($review->entry_criteria) ? implode("\n", $review->entry_criteria) : '';
        $this->exit_criteria_text = is_array($review->exit_criteria) ? implode("\n", $review->exit_criteria) : '';
    }

    #[Computed]
    public function review(): ?Review
    {
        return Review::find($this->reviewId);
    }

    #[Computed]
    public function roleOptions()
    {
        return $this->review?->project->roles()->orderBy('name')->get(['id', 'name']) ?? collect();
    }

    #[Computed]
    public function reviewPlanOptions()
    {
        return $this->review?->project->reviewPlans()->orderBy('name')->get(['id', 'name']) ?? collect();
    }
    ...
    public function save(): void
    {
        ...
        $data = $this->validate([
            ...
            'owner_role_id' => [
                'nullable',
                Rule::exists('roles', 'id')->where('project_id', $review->project_id),
            ],
            'review_plan_id' => [
                'nullable',
                Rule::exists('review_plans', 'id')->where('project_id', $review->project_id),
            ],
...
<flux:modal name="edit-review" :show="$errors->isNotEmpty()" focusable class="max-w-2xl">
    <form wire:submit="save" class="space-y-6">
        <div>
            <flux:heading size="lg">{{ __('Edit review') }}</flux:heading>
        </div>

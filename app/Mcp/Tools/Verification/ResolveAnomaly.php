...
use App\Growth\Transitions\ResolveAnomaly as ResolveAnomalyTransition;
...
#[Description('Resolve an anomaly: move it from investigating to resolved. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class ResolveAnomaly extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new ResolveAnomalyTransition)->apply($anomaly, auth()->user(), $data['reason'] ?? null);

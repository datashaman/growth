...
use App\Growth\Transitions\ReopenAnomaly as ReopenAnomalyTransition;
...
#[IsDestructive(false)]
#[Description('Reopen an anomaly: move it from resolved or closed back to open. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class ReopenAnomaly extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new ReopenAnomalyTransition)->apply($anomaly, auth()->user(), $data['reason'] ?? null);

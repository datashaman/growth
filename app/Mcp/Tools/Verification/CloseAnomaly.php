...
use App\Growth\Transitions\CloseAnomaly as CloseAnomalyTransition;
...
#[IsDestructive(false)]
#[Description('Close an anomaly: move it from resolved to closed. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class CloseAnomaly extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new CloseAnomalyTransition)->apply($anomaly, auth()->user(), $data['reason'] ?? null);

...
use App\Growth\Transitions\CloseRisk as CloseRiskTransition;
...
#[IsDestructive(false)]
#[Description('Close a risk: move it from mitigated, accepted, or realized to closed. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class CloseRisk extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new CloseRiskTransition)->apply($risk, auth()->user(), $data['reason'] ?? null);

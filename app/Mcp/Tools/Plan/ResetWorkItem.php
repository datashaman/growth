...
use App\Growth\Transitions\ResetWorkItem as ResetWorkItemTransition;
...
#[IsDestructive(false)]
#[Description('Reset a work item: move it from in_progress back to todo when work was started by mistake or should return to the backlog. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class ResetWorkItem extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new ResetWorkItemTransition)->apply($workItem, auth()->user(), $data['reason'] ?? null);

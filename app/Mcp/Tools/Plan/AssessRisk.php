...
use App\Growth\Transitions\AssessRisk as AssessRiskTransition;
...
#[Description('Assess a risk: move it from identified to assessed. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class AssessRisk extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new AssessRiskTransition)->apply($risk, auth()->user(), $data['reason'] ?? null);

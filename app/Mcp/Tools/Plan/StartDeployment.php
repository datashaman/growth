...
use App\Growth\Transitions\StartDeployment as StartDeploymentTransition;
...
#[Description('Start a deployment: move it from planned to in_progress. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class StartDeployment extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new StartDeploymentTransition)->apply($deployment, auth()->user(), $data['reason'] ?? null);

...
#[IsDestructive(false)]
#[Description('Assign roles to people (assignee_type=user) or agents (assignee_type=agent). Pass explicit pairs or role_ids with one assignee. Each pair is committed independently and reports attached, already_assigned, or error.')]
class AssignRoles extends Tool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            'role_ids' => $schema->array()
                ->description('Assign one assignee to each listed Role ULID'),
            ...
            'assignee_id' => $schema->string()
                ->description('Identifier of the assignee filling the roles. For assignee_type=user this is the integer user id reported as `user_id` by who-am-i (pass who-am-i\'s `user_id` to self-assign). For assignee_type=agent this is the agent ULID.'),
            'pairs' => $schema->array()
                ->description('Explicit role assignment pairs; each item has role_id, assignee_type, and assignee_id. Pairs commit independently and return per-pair results.'),
        ];
    }

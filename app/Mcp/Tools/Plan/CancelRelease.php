...
use App\Growth\Transitions\CancelRelease as CancelReleaseTransition;
...
#[IsDestructive(false)]
#[Description('Cancel a release before it ships: move it from planned or candidate to cancelled. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class CancelRelease extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new CancelReleaseTransition)->apply($release, auth()->user(), $data['reason'] ?? null);

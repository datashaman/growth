...
use App\Growth\Transitions\MarkDeploymentSucceeded as MarkDeploymentSucceededTransition;
...
#[Description('Mark a deployment as succeeded: move it from in_progress to succeeded. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class MarkDeploymentSucceeded extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new MarkDeploymentSucceededTransition)->apply($deployment, auth()->user(), $data['reason'] ?? null);

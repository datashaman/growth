...
use App\Growth\Transitions\MarkRiskMitigated as MarkRiskMitigatedTransition;
use App\Models\Risk;
...
#[Description('Mark a risk as mitigated: move it from mitigating to mitigated. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class MarkRiskMitigated extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new MarkRiskMitigatedTransition)->apply($risk, auth()->user(), $data['reason'] ?? null);

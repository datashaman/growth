...
use App\Growth\Transitions\MissMilestone as MissMilestoneTransition;
...
#[Description('Mark a milestone as missed: move it from pending to missed. Rejects a milestone that is not pending with a clear message. Records a status transition with the acting user and timestamp.')]
class MissMilestone extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new MissMilestoneTransition)->apply($milestone, auth()->user(), $data['reason'] ?? null);

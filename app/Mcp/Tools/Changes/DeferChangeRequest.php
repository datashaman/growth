...
use App\Growth\Transitions\DeferChangeRequest as DeferChangeRequestTransition;
...
#[IsDestructive(false)]
#[Description('Defer a decision on a change request under review: move it from under_review to deferred and record the deferred decision. Rejects any other source status with a clear message. Records a change approval event with the acting user and timestamp.')]
class DeferChangeRequest extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $event = (new DeferChangeRequestTransition)->apply($changeRequest, auth()->user(), $data['reason'] ?? null);

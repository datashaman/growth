...
use App\Growth\Transitions\ApproveChangeRequest as ApproveChangeRequestTransition;
...
#[IsDestructive(false)]
#[Description('Approve a change request under review: move it from under_review to approved and record the approved decision. Rejects any other source status with a clear message. Records a change approval event with the acting user and timestamp.')]
class ApproveChangeRequest extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $event = (new ApproveChangeRequestTransition)->apply($changeRequest, auth()->user(), $data['reason'] ?? null);

...
use App\Growth\Transitions\MarkChangeRequestImplemented as MarkChangeRequestImplementedTransition;
...
#[IsDestructive(false)]
#[Description('Mark an approved change request as implemented: move it from approved to implemented. Rejects any other source status with a clear message. Records a change approval event with the acting user and timestamp.')]
class MarkChangeRequestImplemented extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $event = (new MarkChangeRequestImplementedTransition)->apply($changeRequest, auth()->user(), $data['reason'] ?? null);
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'reason' => $schema->string()->description('Optional note recorded with the approval event'),
        ];
    }

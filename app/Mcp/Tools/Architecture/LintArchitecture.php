<?php

namespace App\Mcp\Tools\Architecture;

use App\Growth\Alignment\AlignmentText;
use App\Growth\Lint\DesignLinter;
use App\Models\Project;
use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\ResponseFactory;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Lint architecture views for concern coverage, empty views, and unused custom viewpoints.')]
class LintArchitecture extends Tool
{
    public function __construct(private readonly DesignLinter $linter) {}

    public function handle(Request $request): ResponseFactory
    {
        $data = $request->validate([
            'project_id' => 'required|string|owned_project',
        ]);

        $findings = AlignmentText::sanitizeArray($this->linter->check(Project::findOrFail($data['project_id'])));

        return Response::structured([
            'project_id' => $data['project_id'],
            'errors' => collect($findings)->where('severity', 'error')->count(),
            'warnings' => collect($findings)->where('severity', '!=', 'error')->count(),
            'findings' => $findings,
        ]);
    }

    public function schema(JsonSchema $schema): array
    {
        return [
            'project_id' => $schema->string()->description('Project ULID')->required(),
        ];
    }
}

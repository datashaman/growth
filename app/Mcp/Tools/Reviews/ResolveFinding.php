...
use App\Growth\Transitions\ResolveFinding as ResolveFindingTransition;
...
#[IsDestructive(false)]
#[Description('Resolve a review finding: move it from dispositioned to resolved. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class ResolveFinding extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new ResolveFindingTransition)->apply($finding, auth()->user(), $data['reason'] ?? null);

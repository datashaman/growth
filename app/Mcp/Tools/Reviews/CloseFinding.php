...
use App\Growth\Transitions\CloseFinding as CloseFindingTransition;
...
#[Description('Close a review finding: move it from resolved or accepted to closed. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class CloseFinding extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new CloseFindingTransition)->apply($finding, auth()->user(), $data['reason'] ?? null);

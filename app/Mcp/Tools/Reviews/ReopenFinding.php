...
use App\Growth\Transitions\ReopenFinding as ReopenFindingTransition;
...
#[IsDestructive(false)]
#[Description('Reopen a review finding: move it from closed back to open. Rejects any other source status with a clear message. Records a status transition with the acting user and timestamp.')]
class ReopenFinding extends Tool
{
    public function handle(Request $request): ResponseFactory
    {
        ...
        try {
            $transition = (new ReopenFindingTransition)->apply($finding, auth()->user(), $data['reason'] ?? null);

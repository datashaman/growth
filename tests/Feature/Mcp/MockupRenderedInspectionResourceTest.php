...
it('exposes rendered mockup inspection as an mcp resource template', function () {
    ...
    expect(collect($resources)->pluck('uriTemplate')->all())
        ->toContain('growth://mockups/{mockup}/revisions/{revision}/rendered-inspection/{theme}');
});

it('inspects a specific mockup revision through a browser render', function () {
    $theme = Theme::create([
        'project_id' => $this->project->id,
...
HTML);
    ...
    readResource(ReadonlyServer::class, "growth://mockups/{$mockup->id}/revisions/{$revision->id}/rendered-inspection/assigned")
        ->assertOk()
        ->assertSee('"type":"mockup_rendered_inspection"')
...
it('supports disabling theme rendering for inspection', function () {
    ...
    readResource(ReadonlyServer::class, "growth://mockups/{$mockup->id}/revisions/{$revision->id}/rendered-inspection/none")
        ->assertOk()
...
it('errors for revisions that do not belong to the mockup', function () {
    ...
    readResource(ReadonlyServer::class, "growth://mockups/{$mockup->id}/revisions/{$other->currentRevision->id}/rendered-inspection/assigned")
...
it('does not inspect a mockup from another workspace', function () {
    ...
    readResource(ReadonlyServer::class, "growth://mockups/{$mockup->id}/revisions/{$revisionId}/rendered-inspection/assigned")
...
it('is available on the planning server too', function () {
    ...
    readResource(PlanningServer::class, "growth://mockups/{$mockup->id}/revisions/{$revision->id}/rendered-inspection/assigned")
        ->assertOk()

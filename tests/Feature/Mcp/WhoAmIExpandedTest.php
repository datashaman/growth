...
it('reports owned-project count, last-touched project, and role memberships', function () {
    ...
    $older = Project::create(['workspace_id' => $user->active_workspace_id, 'name' => 'Older', 'rigor_level' => 1]);
    $newer = Project::create(['workspace_id' => $user->active_workspace_id, 'name' => 'Newer', 'rigor_level' => 2]);
    ...
    IntakeServer::tool(WhoAmI::class, [])
        ...
        ->assertStructuredContent(function ($json) use ($newer, $older) {
            $json->where('projects_in_workspace', 2)
